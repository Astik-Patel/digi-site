...
function iexcel_scripts_styles() {
	...
	wp_enqueue_style( 'stylesheet', get_template_directory_uri() . '/css/mystyle.css', array(), '2018-02-02' );
}
...
function iexcel_body_class( $classes ) {
	...
	if( rwmb_meta('iexcel_page_nopad') == 1 )
		$classes[] = 'tx-nopad';
...
add_action('admin_notices', 'iexcel_admin_notice_005');
function iexcel_admin_notice_005() {
    global $current_user ;
    ...
    if ( ! get_user_meta($user_id, 'iexcel_ignore_notice_005') ) {
        echo '<div class="updated"><p><div style="line-height: 20px;">'; 
			//printf(__('Welcome to "<b>TemplatesNext\'s i-excel</b>", Kickstart your website from <a href="%1$s">i-excel demo</a> and edit everything instead of creating content from scratch.<br />', 'i-excel'), $demo_import_url);
			...
			printf(__('<a href="%1$s"><span class="dismiss-cross">X</span><span class="dismiss-text">Dismiss this notice</span></a><div class="clear"></div>', 'i-excel' ), '?iexcel_notice_ignore_005=0');				
        echo "</div></p></div>";
    }
}

add_action('admin_init', 'iexcel_notice_ignore_005');
function iexcel_notice_ignore_005() {
    global $current_user;
	$user_id = $current_user->ID;
    /* If user clicks to ignore the notice, add that to their user meta */
	if ( isset($_GET['iexcel_notice_ignore_005']) && '0' == $_GET['iexcel_notice_ignore_005'] ) {
    	add_user_meta($user_id, 'iexcel_ignore_notice_005', 'true', true);
    }
}
