...
add_action( 'wp_enqueue_scripts', 'iexcel_enqueue_scripts' );

function iexcel_enqueue_scripts() {
...
function tx_search_form() {
	return '<form role="search" method="get" class="wpr-search-form" action="' . site_url() . '"><label><input type="search" class="wpr-search-field" placeholder="' . get_theme_mod('search_box_text', __(' Search...', 'i-excel')) . '" value="" name="s" title="Search for:"></label></form>';
}

add_action('wp_footer', 'iexcel_menu', 100);
function iexcel_menu() {
	if( get_theme_mod('enabled', 1) ) :
		...
		<div id="wprmenu_bar" class="wprmenu_bar">
			...
			<div class="menu_title">
				<?php echo esc_attr(get_theme_mod('bar_title', __('MENU', 'i-excel'))); ?>
			</div>
		</div>
...
function iexcel_header_styles() {
	if( get_theme_mod('enabled', 1) ) :
		...
		<style id="wprmenu_css" type="text/css" >
			...
			#wprmenu_bar {
				background: <?php echo esc_attr(get_theme_mod("bar_bgd", "#2e2e2e")); ?>;
			}
...
add_action('wp_head', 'iexcel_header_styles');